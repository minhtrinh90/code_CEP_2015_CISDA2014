...
void userhook_init()
{
    ...
    lat_dist_from_agent3 = 0.0;
    lng_dist_from_agent3 = 0.0;
...
//              set_nav_mode(LOITER_NAV);
      ...
      else if(New_Flight_mode_ON==STATE_CONTROLLED)
      {
          ...
          lat_dist_from_agent1 =0.00001* ((float)(xb_data_f[0].data[0]) - (float)(g_gps->latitude%1000000));
          lng_dist_from_agent1 =0.00001* ( (float)(xb_data_f[0].data[1]) - (float)(g_gps->longitude%1000000));
          
          lat_dist_from_agent2 = 0.00001* ((float)(xb_data_f[1].data[0]) - (float)(g_gps->latitude%1000000));
          lng_dist_from_agent2 = 0.00001* ((float)(xb_data_f[1].data[1]) - (float)(g_gps->longitude%1000000));
          
          lat_dist_from_agent3 = 0.00001* ((float)(xb_data_f[2].data[0]) - (float)(g_gps->latitude%1000000));
          lng_dist_from_agent3 = 0.00001* ((float)(xb_data_f[2].data[1]) - (float)(g_gps->longitude%1000000));         
          
          bearing_x_1 = lat_dist_from_agent1/sqrt(lat_dist_from_agent1*lat_dist_from_agent1 + lng_dist_from_agent1*lng_dist_from_agent1);
          bearing_y_1 = lng_dist_from_agent1/sqrt(lat_dist_from_agent1*lat_dist_from_agent1 + lng_dist_from_agent1*lng_dist_from_agent1);
          
          bearing_x_2 = lat_dist_from_agent2/sqrt(lat_dist_from_agent2*lat_dist_from_agent2 + lng_dist_from_agent2*lng_dist_from_agent2);
          bearing_y_2 = lng_dist_from_agent2/sqrt(lat_dist_from_agent2*lat_dist_from_agent2 + lng_dist_from_agent2*lng_dist_from_agent2);
          
          bearing_x_3 = lat_dist_from_agent3/sqrt(lat_dist_from_agent3*lat_dist_from_agent3 + lng_dist_from_agent3*lng_dist_from_agent3);
          bearing_y_3 = lng_dist_from_agent3/sqrt(lat_dist_from_agent3*lat_dist_from_agent3 + lng_dist_from_agent3*lng_dist_from_agent3);  
          
          angle1C = findAngle(bearing_x_1, bearing_y_1, bearing_x_2, bearing_y_2);
          angle1A = findAngle(bearing_x_2, bearing_y_2, bearing_x_3, bearing_y_3);
          angle1B = findAngle(bearing_x_3, bearing_y_3, bearing_x_1, bearing_y_1); 

          if (abs(angle1C + angle1B + angle1A - 2*PI) < 0.0001)
          {
             control_input_x = -30*((angle1Cd - angle1C) * bearing_x_3 + (angle1Ad - angle1A) * bearing_x_1 + (angle1Bd - angle1B) * bearing_x_2);
             control_input_y = -30*((angle1Cd - angle1C) * bearing_y_3 + (angle1Ad - angle1A) * bearing_y_1 + (angle1Bd - angle1B) * bearing_y_2);
          }
          else
          {
             control_input_x = 30*(bearing_x_3 + bearing_x_2 + bearing_x_1);
             control_input_y = 30*(bearing_y_3 + bearing_y_2 + bearing_y_1);           
          }
...
//          xbee_data_to_packet(AGENT_5,GPS_DATA);
//          }
          if (sqrt(control_input_x * control_input_x + control_input_y * control_input_y) <= 4)
          {
              my_count++;
//            control_input_x = 0.0;
//            control_input_y = 0.0;
