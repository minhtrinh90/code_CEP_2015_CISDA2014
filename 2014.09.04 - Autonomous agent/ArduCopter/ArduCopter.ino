...
bool set_throttle_mode( uint8_t new_throttle_mode )
{
    ...
    switch(new_throttle_mode) {
        ...
        case THROTTLE_HOLD_USER:
            //controller_desired_alt = get_initial_alt_hold(current_loc.alt, 0.0);     // reset controller desired altitude to current altitude
            wp_nav.set_desired_alt(400.0);                                 // same as above but for loiter controller
            if ( throttle_mode <= THROTTLE_MANUAL_TILT_COMPENSATED ) {      // reset the alt hold I terms if previous throttle mode was manual
                reset_throttle_I();
                set_accel_throttle_I_from_pilot_throttle(0);
            }
            throttle_initialised = true;
            break;
...
void update_throttle_mode(void)
{
    ...
    case THROTTLE_HOLD_USER:
        if(ap.auto_armed) {
            // alt hold plus pilot input of climb rate
            pilot_climb_rate = 0;//get_pilot_desired_climb_rate(g.rc_3.control_in);
            if( sonar_alt_health >= SONAR_ALT_HEALTH_MAX ) {
                // if sonar is ok, use surface tracking
                get_throttle_surface_tracking(pilot_climb_rate);    // this function calls set_target_alt_for_reporting for us
            }else{
